...
export function completionLine(obj: Objective): string {
  ...
  // Engine dates are UTC-midnight instants; render as that calendar day on every engine/timezone
  // (formatUTCDate is Hermes-safe — no Intl timeZone reliance, so it's correct on native too).
  const dateStr = formatUTCDate(on, { day: 'numeric', month: 'short', year: 'numeric' });
...
export function formatTiming(obj: Objective): string {
  const t = obj.timing;
  // Hermes-safe UTC-day render — see completionLine / lib/utcDate.
  const fmt = (d: Date) => formatUTCDate(d, { day: 'numeric', month: 'short', year: 'numeric' });
