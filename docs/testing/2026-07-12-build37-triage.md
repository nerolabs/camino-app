...
| 7 | Andrew (screenshot) | Share dialog styling broken on web — prose floating, button shrink-wrapped with text at its edges | The celebrate-card centers children; the share dialog inherited that | both | **FIXED** (stretch layout, caveat callout, full-width cobalt button) |
