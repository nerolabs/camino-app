...
| **Interview bubble emphasis** | `lola-bubble.test.ts` (6) | `splitLolaBubble` separates preamble from the trailing question and flags it only when it reads as a question (ends `?`, incl. Spanish `¿…?`); ack-only / statement bubbles stay plain; splits on the LAST blank line so multi-paragraph preambles keep their break |
