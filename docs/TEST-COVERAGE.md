...
| **Turnstile session tokens** (C2b) | `session.test.ts` (9) | `mintSession`/`verifySession` HMAC (round-trip, expiry, wrong-secret, tampered sig/bumped-exp, null/garbage); `sessionGate` enforce-when-configured / 401 on missing-or-bad / pass on valid / fail-open on missing signing secret |
